...
function renderTeam() {
    if (teamNameDisplay) teamNameDisplay.textContent = team.name;
...
window.handleSaveTeam = function() {
    ...
    const success = editTeamName(team, newName);
    if (success) {
        saveToStorage();
        teamModal.classList.remove('open');
    }
...
window.handleSavePlayer = function() {
    ...
    if (id === "") {
        const success = addNewPlayer(team, player);
        if (success) {
            saveToStorage();
            playerModal.classList.remove('open');
        }
    } else {
        const success = updatePlayer(team, player);
        if (success) {
            saveToStorage();
            playerModal.classList.remove('open');
        }
...
window.handleDeletePlayer = function() {
    if (confirm("Do you really want to delete this player?")) {
        team = deletePlayer(team, inpId.value);
        saveToStorage();
        playerModal.classList.remove('open');
    }
};
